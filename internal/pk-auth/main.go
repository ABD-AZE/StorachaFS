...
// Global cache for clients (shared between email and private key auth)
var cachedClients = make(map[string]*client.Client)

// EmailAuth - existing email authentication
...
func PrivateKeyAuth(config *AuthConfig) (*client.Client, error) {
	// Create cache key based on file paths and space DID
